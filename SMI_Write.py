...
#MetadataLen = Metadata Length
    # write pattern=0x5a
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null | nvme write %s -s 0 -z 512 -y %s 2>&1"%(mNVME.dev,MetadataLen))
...
def testDW10_DW11(SLBA, msg0, msg1 , ExpectCommandSuccess):  
    ...
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null |nvme io-passthru %s -o 0x1 -n 1 -l 512 -w --cdw10=%s --cdw11=%s 2>&1"%(mNVME.dev, cdw10, cdw11))
...
def testDW12(LR, FUA, PRINFO, NLB, ExpectCommandSuccess):      
    ...
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null |nvme io-passthru %s -o 0x1 -n 1 -l 512 -w --cdw10=%s --cdw11=%s --cdw12=%s 2>&1"%(mNVME.dev, cdw10, cdw11, cdw12))
    retCommandSueess=bool(re.search("NVMe command result:00000000", mStr))
    if (retCommandSueess ==  ExpectCommandSuccess) :        
        return True         
    else:
...
def testDW12NLB(msg0, msg1 ,NLB, ExpectCommandSuccess):      
    ...
    print msg1   
    cdw12=NLB
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null |nvme io-passthru %s -o 0x1 -n 1 -l 512 -w --cdw10=%s --cdw11=%s --cdw12=%s 2>&1"%(mNVME.dev, 0, 0, cdw12))
    retCommandSueess=bool(re.search("NVMe command result:00000000", mStr))
    if (retCommandSueess ==  ExpectCommandSuccess) :
        mNVME.Print("PASS", "p")  
        return True         
...
def testDW13(DSM, ExpectCommandSuccess):      
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null |nvme io-passthru %s -o 0x1 -n 1 -l 512 -w --cdw13=%s 2>&1"%(mNVME.dev, DSM))
    retCommandSueess=bool(re.search("NVMe command result:00000000", mStr))
    if (retCommandSueess ==  ExpectCommandSuccess) :
        return True         
    else:
...
def testDW14(EILBRT, ExpectCommandSuccess):      
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null |nvme io-passthru %s -o 0x1 -n 1 -l 512 -w --cdw14=%s 2>&1"%(mNVME.dev, EILBRT))
    retCommandSueess=bool(re.search("NVMe command result:00000000", mStr))
    if (retCommandSueess ==  ExpectCommandSuccess) :
        return True         
    else:
...
def testDW15(LBATM,LBAT, ExpectCommandSuccess):      
    CDW15=(LBATM<<16)+LBAT
    mStr=mNVME.shell_cmd("dd if=/dev/zero bs=512 count=1 2>&1   |tr \\\\000 \\\\132 2>/dev/null |nvme io-passthru %s -o 0x1 -n 1 -l 512 -w --cdw15=%s 2>&1"%(mNVME.dev, CDW15))
    retCommandSueess=bool(re.search("NVMe command result:00000000", mStr))
    if (retCommandSueess ==  ExpectCommandSuccess) :
        return True         
    else:
...
for i in range(16):  
    ...
    ML=i
    msg0="set Metadata Length=%s for write command"%(ML)
...
for i in range(0x100):
    LBATM= (i<<8) + i
    LBAT= (i<<8) + i
    ret_code=ret_code if testDW15(LBATM,LBAT, True) else 1
