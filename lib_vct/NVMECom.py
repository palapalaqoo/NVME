...
class NVMECom():   
    ...
    def get_log_passthru(self, LID, size, RAE=0, LSP=0):
    #-- return list [ byte[0], byte[1], byte[2], ... ]
    #-- size, size in bytes
    #-- usage: mStr=get_log2byte(7, 512)=[01, 23, 45, 67, 89]
    #-- mStr[0]='01' , mStr[1]='23'  
        ...
        cmd="nvme admin-passthru %s --opcode=0x2 -r --cdw10=%s --cdw11=%s -l %s 2>&1 "%(NVMECom.device, CDW10, NUMDU, size)
        mbuf=self.shell_cmd(cmd)
        line="0"
        # if command success
        if re.search("NVMe command result:00000000", mbuf):
            patten=re.findall("[\s\d{2}]{48}", mbuf)
            patten1= ''.join(patten)
