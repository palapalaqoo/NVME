...
class SMI_SRIOV(NVME):
    ...
    def TestWriteFormatRead(self, SubDUT, writeValue):
        ...
        mStr=self.shell_cmd(" nvme format %s -n %s -l %s -s %s -p %s -i %s -m %s 2>&1"%(SubDUT.dev, nsid, lbaf, ses, pil, pi, ms))
    ...
    def TestSpecificVFandOtherVFshouldNotBeModified(self, SpecificDevice):
        ...
        self.Print("")        
        # create SubDUT to use NVME object for specific device, e.x. /dev/nvme0n1        
    ...
    def ThreadTest(self, device, testTime): 
        ...
        # create SubDUT to use NVME object for specific device, e.x. /dev/nvme2n1
        SubDUT = NVME([device])    
